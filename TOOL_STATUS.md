...
- [x] `loop_get_team_membership` - tested: yes
- [x] `loop_list_teams` - tested: yes
- [x] `loop_list_team_public_channels` - tested: yes
...
- [x] `loop_list_my_team_channels` - tested: yes
- [x] `loop_get_channel_stats` - tested: yes
- [x] `loop_list_team_channel_memberships` - tested: yes
- [x] `loop_list_channel_members` - tested: yes
- [x] `loop_get_channel_membership` - tested: yes
...
- Latest detail smoke also validated `loop_get_user`, `loop_get_team`, `loop_get_channel`, `loop_get_channel_stats`, `loop_list_channel_members`, `loop_get_channel_membership`, `loop_get_post`, and `loop_create_direct_channel`.
- Latest batch2 smoke also validated `loop_get_users_by_ids`, `loop_list_team_members`, `loop_list_team_channel_memberships`, `loop_list_pinned_posts`, and `loop_create_group_channel`.
- Latest batch3 smoke also validated `loop_get_team_membership`, `loop_resolve_team_by_name`, `loop_get_users_by_usernames`, `loop_get_user_statuses_by_ids`, and `loop_list_my_team_unreads`.
...
- Latest scenario suite also validated that `loop_list_my_team_channels` resolves direct-message counterpart names for `type: "D"` channels.
- Latest scenario suite also validates the distinction between `loop_list_my_team_channels` and `loop_list_team_public_channels`.
- Latest scenario suite also validates finding public team channels that the current user has not joined.
